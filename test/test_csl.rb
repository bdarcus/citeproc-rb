...
class TestCsl < Test::Unit::TestCase
  ...
  def test_group
    ...
    # Get the second group node
    group_node = second_node.children[0]
    results = @formatter.format(group_node)
    assert(results.empty?)

    group_node = second_node.children[1]
    results = @formatter.format(group_node)
    assert_equal("The words 'Splitting the Difference' appear in the citation.", results)
  end
